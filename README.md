...
That is not a small difference. That is a full-on transformation.
...
These numbers are wild considering the hardware.
...
Yes. I somehow turned a simple arithmetic typo into a large-scale stress test.
...
./build/bin/llama-cli \
  -m ~/ai/models/gemma-4b-it-Q5_K_M.gguf \
  -t 6 \
  -c 8192 \
  2>&1 | tee ~/llm_chats/chat_$(date +%F_%H-%M-%S).txt
...
4. AVX2 makes CPUs surprisingly capable.
...
Screenshots of benchmark runs, TTY outputs, and code generation examples will be added here.
